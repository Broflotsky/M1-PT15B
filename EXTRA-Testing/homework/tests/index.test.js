...
describe('checkSeatStatus test', () => {
    ...
    // TEST 3
    it('El segundo parametro debe ser un número', () => {
        expect(() => checkSeatStatus('A', '2')).toThrow(new TypeError('El segundo parametro debe ser un número'))
    })

    // TEST 5
    it('Deberia retornar true si la silla esta reservada', () => {
        expect(checkSeatStatus('A', 1)).toBe(true)
    })

    it('Deberia retornar false si la silla no esta reservada', () => {
        expect(checkSeatStatus('E', 3)).toBe(false)
    })
})

describe('getRowNumber test', () => {
    // TEST 4
